...
async def artillery_chat(request: ChatRequest):
    """Chat with legal documents using Artillery RAG system."""
    import time
    import traceback
    start_time = time.time()
    ...
    try:
        ...
        # Build simple prompt
        system_prompt = """You are LeguBot, a helpful legal information assistant. Answer questions clearly and conversationally, like ChatGPT. Start with a direct answer to the question."""
        ...
        logger.info(f"[ARTILLERY_CHAT] Returning response with answer length: {len(answer)}")
        return ChatResponse(
            answer=answer[:5000],
            citations=[],
            chunks_used=0,
